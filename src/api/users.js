...
import { baseUrlFrom, checkIfMatch, listResponse, parseListParams, scimFilterToMongo, sendError, setCommonHeaders } from './util.js';
...
usersRouter.get('/Users', async (req, res) => {
    ...
    const resp = listResponse(base, 'Users', resources, total, startIndex, count);
...
usersRouter.post('/Users', async (req, res) => {
    ...
    if (!body.userName)
        return sendError(res, 400, 'userName is required', 'invalidValue');
...
usersRouter.put('/Users/:id', async (req, res) => {
    ...
    if (!checkIfMatch(req, currentVersion))
        return sendError(res, 412, 'Precondition Failed', 'mutability');
    const body = sanitizeUserInput(req.body);
...
function applyPatch(target, op) {
    ...
    if (op.op === 'remove') {
        if (Array.isArray(obj[last]) && typeof op.value === 'object') {
            obj[last] = obj[last].filter((v) => JSON.stringify(v) !== JSON.stringify(op.value));
        }
        else {
            delete obj[last];
        }
...
usersRouter.patch('/Users/:id', async (req, res) => {
    ...
    if (!Array.isArray(body?.Operations))
        return sendError(res, 400, 'Invalid PatchOp', 'invalidSyntax');
    const col = collection();
    const existing = await col.findOne({ _id: new ObjectId(req.params.id) });
    if (!existing)
        return sendError(res, 404, 'User not found');
