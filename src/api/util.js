...
export function makeMeta(resourceType, id, baseUrl, versionNum, created) {
...
export function toScim(resourceType, doc, baseUrl) {
    if (!doc)
        return null;
...
export function listResponse(baseUrl, resourceType, resources, totalResults, startIndex, count) {
    return {
...
export function error(status, detail, scimType) {
    return {
...
export function sendError(res, status, detail, scimType) {
...
export function scimFilterToMongo(filter) {
    if (!filter)
        return {};
...
function pathToMongo(path, value) {
...
export function etagNumberFrom(header) {
    if (!header)
        return undefined;
...
export function checkIfMatch(req, currentVersion) {
...
export function parseListParams(req) {
    ...
    const sortOrder = sortOrderParam?.toLowerCase() === 'descending' ? -1 : 1;
